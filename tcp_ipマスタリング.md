...
代理ARPは異なるセグメントに対するARP要求パケットが流れ、ルーターが自分のMACアドレスを返す  
その結果IPパケットはルーターに送られることになり、ルーターは受け取ったIPパケットを本来のIPアドレスが付いているノードに転送する  
つまり異なる2つのセグメントがまるで同一セグメンドであるかのように振る舞える  
これは、サブネットマスクやルーティングテーブルを定義できない機器がある場合や、複数のサブネットを重ねて使用するVPN環境などで代理ARPが利用されることがある  
## 5.4 ICMP(Internet Control Message Protocol)
### 5.4.1 IPを補助するICMP
IPネットワークを構築したときに大切になるのが、ネットワークが正常に動作しているかどうかの確認と、以上が発生したときのトラブルシューティングである  
たとえば、ネットワークを構築したときには、設定が正しいかどうかを確認する手段が必要です  
また、ネットワークがうまく動作しない場合に、何が問題なのかを突き止める手段も必要になる  
管理者の苦労を減らすためには、これらの機能が必要になる  
それを解決するプロトコルがICMPである  
ICMPには、IPパケットが目的のホストまで届くかどうかを確認する機能や、何らかの原因でIPパケットが廃棄されたときにその原因を通知してくれる機能、不十分な設定をよりよい設定に変更してくれる機能がある  
ICMPには大きく分類するとエラー通知のためのエラーメッセージと診断などを行う問い合わせメッセージの2種類がある  
## 5.5 DHCP(Dynamic Host Configuration Protocol)
### 5.5.1 プラグ＆プレイを可能にするDHCP
一つ一つのホストに面倒なIPアドレスの設定を自動化したり、配布するIPアドレスの一括管理を行ってくれるのがDHCPが利用される  
なので、コンピュータを繋いだだけでネットワークに接続できる  
### 5.5.2 DHCPの仕組み
DHCPをつかうためにDHCPサーバーを立て、配布するIPアドレスを設定する必要がある  
## 5.6 NAT(Network Address Translator)
### 5.6.1 NATとは
NAT(Network Address Translator)は、プライベートIPアドレスをグローバルIPアドレスへ変換する技術として開発された  
さらに発展して、TCPやUDPのポート番号も付け替えるNAPT（Network Address Ports Translator）が登場したことで1つのグローバルIPアドレスで複数のホスト間の通信が可能になった  
### 5.6.2 NATの仕組み
NAT対応のルーターの内部ではアドレス変換のためのテーブルが自動的に作られる  
例えば10.0.0.10から163.221.120.9にむけて最初にパケットが送られたときに変換テーブルが作られ、それに対応した処理が行われる  
### 5.6.4 CGN(Carrier Grade NAT)
CGNはISPレベルでNATを行う技術である  
ISPは顧客に対してグローバルIPv4Addressを一つずつ割り当てるが、インターネットの爆発的な普及でそれができなくなっている  
そこで、CGNを使ってISPは顧客に対し**プライベートIPv4アドレス**を配布する  
そして、インターネットと通信するときは、NATによる変換を2回する  
1. 各組織のプライベートIPアドレス <=> ISPが各組織に配布したプライベートIPアドレス  
2. ISPが各組織に配布したプライベートIPアドレス <=> グローバルIPアドレス  
## 5.7 IPトンネリング
ネットワークのなかで、とあるセグメントA,BではIPv4、とあるセグメントCではIPv6のネットワークを使っているとA->C->Bと通信しようとすると互換性はないので、成立しない  
それをIPトンネリングはネットワークAやBで流れるIPv4パケット全体を一つのデータとして扱い、その前にIPv6ヘッダを付けることでネットワークCを通過できるようにする  
つまり、IPトンネリングによってネットワークCをトンネルのように通過することでネットワークA<->Bの通信が可能になる  
つまりIPv4ヘッダがあるパケットにIPv6ヘッダがつくという、ネットワーク層のヘッダの次にまたネットワーク層のヘッダが続く通信方法が**IPトンネリング**である  
VPNでもIPトンネリングが使われる、なるほど!!!  
通常ではマルチキャストを通過しないルーターに、トンネリングをつかうことでルーターを通過するユニキャストパケットにカプセル化することで、遠く離れたリンクまでマルチキャストパケットを送信することができる  
ただ、IPv4とIPv6の両方に対応するネットワークの構築は大変手間である  
### 5.8.2 IPマルチキャスト関連技術
マルチキャストはUDPを使う  
このため、マルチキャストにとって重要なのは受信者がいるのかどうかの確認が重要  
受信者がいるかどうかの通知にはIPv4ではIGMP、IPv6ではICMPv6の機能の一つであるMLDが使われる  
### 5.8.3 IPエニーキャスト
IPエニーキャストは電話でいうと110番（警察）や119(消防)のような機能を提供する  
IPエニーキャストで有名なサービスはDNSのルートネームサーバーである  
# 6. TCPとUDP
## 6.1 トランスポート層の役割
### 6.1.2 通信の処理
HTTPや、sshなどを処理するサーバープログラムはUNIXではデーモンと呼ばれる  
HTTPならhttpd(HTTPデーモン)、sshのサーバーはsshd（sshデーモン）である  
リクエストがそのデーモンで処理するのかはパケットの宛先ポート番号を調べればいい  
:80ならhttpdにコネクションを確立させて、そのデーモンがそれ以降の処理を受け持つ  
#### (余談)ソケット
TCPやUDPではソケットと呼ばれるAPIが広く使われる  
アプリケーションはソケットを利用して、通信相手のIPアドレスやポート番号の設定、データの送受信や受信の要求を行う  
するとOSがTCP/IPによる通信を行ってくれる  
## 6.2 ポート番号
### 6.2.3 IPアドレスとポート番号とプロトコル番号による通信の識別
通信の識別にはポート番号だけでは識別できないので、送信元のアドレスとプロトコルとポート番号で識別している  
ポート番号には、標準的に決まっている番号とダイナミック(動的)な割当法の2つがある  
#### ダイナミック(動的)な割当法
サーバー側はポート番号が決まっていないといけないが、クライアントは別に決まっている必要はない  
なので、クライアントアプリケーションはOSにポート番号を決めさせる  
OSはアプリケーションごとに同じ値にならないように制御しながらポート番号を割り当てる  
### 6.4.2 シーケンス番号と確認応答で信頼性を提供
TCPでは送信したデータが受信ホストに到達したとき、受信ホストは送信ホストにデータが到達したことを知らせる、これを確認応答（ACK）という  
(ACKは「アック」といえば肯定確認応答の意味になる)  
ACKは会話で言う相手に「うん、うん」と相づちをしてACKを返すようなもの  
逆に「え？」といって受信できなかったことを相手に伝える否定確認応答（NACK）という  
ということで、TCPはACKでデータ到達の信頼性を実現する  
### 6.4.4 コネクション管理
TCPはデータ通信前にTCPヘッダからなるコネクション確立要求のパケット（SYNパケット）を送る、そして相手が応答したときに初めてデータ通信が可能になる  
### 6.4.5 TCPはセグメント単位でデータを送信
IPではMTUを基準にフラグメンテーションをしたパケットを送信していたように、TCPも最大セグメント長（MSS）と呼ばれるデータ単位で送信する  
### 6.4.10 ネットワークの利用効率を高める仕組み
#### ピギーバック
アプリケーションプロトコルによっては、確認応答(ACK)とデータの送信を同時に送るピギーバックという仕組みで通信することでネットワーク通信の効率を上げることができる  
ピギーバックは、語源は町に豚を売りに行くついでに豚の背中に野菜を乗せて売りにいくという「ついでに運ぶ」ニュアンスがある  
SMTPはPOP,ファイル転送のFTPの制御コネクションなどこれが使われる  
HTTPも1.1から使われているらしい  
# 7 ルーティングプロトコル
## 7.1 経路制御（ルーティング）とは
### 7.1.3 ダイナミックルーティングの基礎
ダイナミックルーティングは隣り合うルーター間で自分が知っているネットワークの接続情報を教え合うことで伝われる  
情報がバケツリレー式で伝わり、ネットーワークの隅々まで伝わればルーティングテーブルが完成し、IPパケットを正しく転送できるようになる  
## 7.3 経路制御アルゴリズム
### 7.3.1 距離ベクトル型（Distance-Vector）
距離ベクトル型のアルゴリズムとは、距離（メトリック）と方向によって目的のネットワークやホストの位置を決定する方法である  
### 7.3.2 リンク状態型（Link-State）
リンク状態型は、ルーターがネットワーク全体の接続状態を理解して経路制御表を作成する方法である  
なので、他のルーターと同じ経路制御情報をもっていれば、正しい情報を持っていることになるので、安定したルーティングを行える  
その代償としてネットワークトポロジーからルーティングテーブルを求める計算はかなり複雑  
特にネットワーク規模が大きくなるとトポロジー情報の管理や処理をするために高いCPU能力と多くのメモリ資源が必要になる  
# 8 アプリケーションプロトコル
## 8.2 遠隔ログイン（TELNETとSSH）
### 8.2.1 TELNET
TELNETはTCPのコネクションを利用する  
この通信路を通して相手のコンピュータにコマンドが文字列として送信され、相手のコンピュータで実行される  
自分のキーボードとディスプレイが、相手のコンピュータの内部で動作しているシェルに接続しているイメージ  
### 8.2.2 SSH
ポートフォワーディングとは特定のポート番号に届けられたメッセージを、特定のIPアドレス、ポート番号に転送する仕組み  
### 8.4.1 電子メールの仕組み
従来のメールはTCPで直接接続してメールのやりとりをしていた  
そうすると、相手のコンピュータが電源OFFしていると、メールの送信ができず後で再送していた  
そうではなく、間にメールサーバーを経由することで相手のコンピュータの都合を考えることなくメールを送ることができた  
そして、受信者がメールサーバーからメールを受け取るPOP（Post Office Protocol）というプロトコルが標準化された  
### 8.4.3 MIME（Multipurpose Internet Mail Extensions）
電子メールで送る形式はMIMEが一般的になったことで、画像や動画を添付して送ることができる  
MIMEは基本的にヘッダと本文（データ）の2つの部分から構成される  
ヘッダには空行があってはならず、空業があるとそこからデータとみなされる  
MIMEヘッダの**Content-Type**で**Multipart/Mixed**を指定し、**boundary=** のあとに書いた文字列を集合として定義する  
Content-Typeはヘッダに続く情報がどのような種類のデータなのかを示している  
### 8.5.3 URI(Uniform Resource Identifier)
URLはインターネットの資源（ファイルなど）の場所を表す俗称  
URIは**インターネットの資源に限らず**あらゆる資源を識別できるように考えられた識別子  
現在有効なRFC文書はURIが使われる  
URLはインターネット限定の資源を表すが、URIはwww以外のアプリケーションプロトコルでも使うことができる  
URIが表す枠組みをスキームという  
スキーム://ホスト名:ポート番号/path
#### CGI
CGIは、Webサーバーが外部プログラムを呼び出すサーバーサイドアプリケーションの仕組み  
#### WebSocket
クライアントとサーバーの双方向通信をHTTPで実現するプロトコル  
もともとHTTPは片方向通信を想定して作られたプロトコルだが、さまざまなアプリケーションで双方向のリアルタイム通信が必要とされ始めたことが背景にある  
(というかHTTPって片方向だったんだ)
# 9 セキュリティ
## 9.2 セキュリティの構成要素
## 9.2.2 IDS/IPS(侵入検知システム/侵入防止システム)
ファイアウォールは基本的にポリシーと合致した通信であれば悪意があるアクセスかどうか関係なく通過させる  
なので、IDS(侵入検知システム)を使って内部に侵入した不正アクセス通信をみつけ、セキュリティの管理者などに通知をする  
